...
// Client-side glue: keyboard handling for the block editor, draft
// preservation across collaborative re-renders, and drag & drop. Everything
// else (state, rendering, persistence) lives on the server.
const glueJS = `
<script>
(function(){
...
.nt-logo { padding: 14px 16px; font-weight: 700; font-size: .95rem; display: flex; gap: 8px; align-items: center; }
.nt-tree { flex: 1; overflow-y: auto; padding: 4px; }
...
.nt-page { max-width: 720px; margin: 0 auto; padding: 40px 24px 30vh; }
.nt-icon { font-size: 3.2rem; cursor: pointer; width: fit-content; border-radius: 8px; padding: 2px 6px; }
.nt-icon:hover { background: #f1f0ee; }
.nt-iconmenu { background: #fff; border: 1px solid #e3e2e0; border-radius: 10px; box-shadow: 0 8px 24px rgba(0,0,0,.12); padding: 8px; display: inline-block; }
...
.nt-gut { width: 44px; display: flex; justify-content: flex-end; gap: 2px; padding-top: 5px; visibility: hidden; flex-shrink: 0; }
.nt-blk:hover .nt-gut { visibility: visible; }
.nt-plus, .nt-handle { color: #b9b8b5; cursor: pointer; border-radius: 4px; padding: 0 3px; font-size: .85rem; user-select: none; }
.nt-plus:hover, .nt-handle:hover { background: #ececea; color: #5f5e5b; }
...
.nt-input { width: 100%; border: none; outline: none; resize: none; font-family: inherit; font-size: .95rem; line-height: 1.6; background: #f6f9ff; border-radius: 4px; padding: 2px 4px; color: #37352f; overflow: hidden; }
...
.nt-menu { position: absolute; left: 44px; top: 26px; background: #fff; border: 1px solid #e3e2e0; border-radius: 10px; box-shadow: 0 10px 30px rgba(0,0,0,.15); z-index: 50; min-width: 180px; padding: 5px; }
.nt-menu-item { padding: 6px 10px; border-radius: 6px; cursor: pointer; font-size: .88rem; }
